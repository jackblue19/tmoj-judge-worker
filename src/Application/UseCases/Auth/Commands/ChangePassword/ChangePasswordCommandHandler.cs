...
public class ChangePasswordCommandHandler : IRequestHandler<ChangePasswordCommand , string>
{
    ...
    public async Task<string> Handle(ChangePasswordCommand request , CancellationToken ct)
    {
        ...
        var apiBaseUrl = _config["EmailSettings:ApiBaseUrl"]?.TrimEnd('/')
                        ?? throw new InvalidOperationException("EmailSettings:ApiBaseUrl is required.");

        var confirmLink =
            $"{apiBaseUrl}/api/v1/Auth/confirm-email" +
