...
public sealed class GetTestsetPreviewQueryHandler
    : IRequestHandler<GetTestsetPreviewQuery , TestcaseContentListDto>
{
    ...
    public GetTestsetPreviewQueryHandler(
    ...
    public async Task<TestcaseContentListDto> Handle(GetTestsetPreviewQuery request , CancellationToken ct)
    {
        EnsureAuthenticated();
        ...
        var testcases = await _testcaseRepo.ListAsync(
            new TestcasesByTestsetSpec(request.TestsetId , 3) , ct);

        var items = new List<TestcaseContentItemDto>();
